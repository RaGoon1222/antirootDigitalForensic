...
#### 커널 오브젝트 검색 방법
다음은 메모리에서 프로세스를 찾는 방법을 설명한 것이다.  
사실 프로세스를 찾는 방법이라고는 했지만 작게 보면 프로세스이고, 크게 보면 커널 오브젝트에도 해당 방법들이 적용된다.  

이런 방법들을 바탕으로 현재 사용되고 있는 메모리 분석 도구가 개발되었다.  
대부분이 메모리 분석 도구는 다음에 설명하는 세 가지 방법 모두 조사관에게 기본 기능으로 제공한다.  

1. 리스트 워킹  
하나의 단서를 근거로 목적지까지 거슬러 오르는 방법이다.  
위에서 설명했었듯이 어떤 프로세스의 ActiveProcessLink를 알고 있다면 앞이나 뒤의 프로세스를 계속 찾다 보면 결국 다시 처음 출발하였던 곳으로 돌아오므로 그동안 거쳐왔던 프로세스를 나열하면 프로세스 목록이 얻어지는 것이다.  
하지만 위에서 설명한 DKOM 기법을 이용하면 이 검색방법으로는 숨겨진 프로세스를 찾을 수 없다.  
2. 또 다른 리스트 워킹  
사실 명칭은 같지만 방식이 다른 이 리스트 워킹 방법은 EPROCESS가 아닌 또 다른 커널 구조체인 KPCR을 기초로 한다.  
KPCR은 XP, 2003에서 가상 주소 0xFFDFF000에 위치하며 KPCR의 확장 영역인 KPCRB는 같은 버전에서 0xFFDFF120에 위치한다.  
KPCRB는 EPROCESS의 KTHREAD 구조체에 대한 포인터 값이 들어있어 KPCRB를 통해 KTHREAD를 알면 EPROCESS의 위치도 알 수 있다.  
또 이 방법은 위 기존의 리스트 워킹 방법과 마찬가지로 DKOM 기법이 적용된 프로세스는 찾지 못하며, Vista 이후의 버전들의 KPCR 주소가 컴퓨터마다 달라 Vista 이후 버전에는 적용하기 힘들다.
3. 패턴 매칭  
리스트 워킹의 한계 극복을 위해 개발된 기법이다.  
해당 방법의 기본 원리는 Windows의 프로세스와 스레드의 일정 구조를 이용한 것이다.  
예를 들어, 모든 커널 오브젝트가 OBJECT_HEADER라는 구조체를 공통적으로 가지고 있고, 이를 이용해 메모리에서 오브젝트를 찾을 수 있는 것처럼 말이다.  
정확한 패턴매칭과 룰은 논문에서 확인할 수 있으니 한번 참고해 보기 바란다.  

### 메모리 분석  
이전의 메모리 분석은 문자열 검색이나 [파일 카빙](https://en.wikipedia.org/wiki/File_carving)정도로 그쳤다.  
하지만 요즘은 메모리에 대한 연구로 메모리에서 추출할 수 있는 데이터와 그 방법들이 많아졌다.  
그 결과, 전문 도구까지 사용자들에게 배포되었다.  
메모리 분석 도구는 요즘 너무 잘 나온 것들이 많아 일반적으로 사용하는 무료 소프트웨어인 Volatility와 redline에 대해 소개할 것이다.  

#### Volatility  
해당 도구는 다양한 기능과 플러그인을 제공하는 CLI도구이며, 파이썬 기반으로 만들어졌다.  
Volatility에서 프로세스 목록을 얻는 방법은 두 가지가 있다.  
하나는 pslist 옵션으로 얻는 방법과 psscan 옵션으로 얻는 방법이 있는데, 두 옵션에 결과는 덤프 이미지에 따라 다를 수 있다. pslist의 경우 리스트워킹 방식을 사용하기 때문에 숨겨진 프로세스를 찾지 못하는 반면, psscan 옵션은 [패턴 매칭 방식](https://en.wikipedia.org/wiki/Pattern_matching)을 사용하여 숨겨진 프로세스까지 찾아 준다.  
속도면에서는 pslist가 더 빠르긴 하지만 결과물면에서는 psscan이 더 좋다.  

![pslist&psscan](https://github.com/RaGoon1222/antirootDigitalForensic/blob/master/week_1/img/pslist&psscan.PNG?raw=true)  

이번에는 Live Response에서도 보았던 네트워크 연결 정보이다.  
connections, connscan 옵션을 사용하면 정보를 얻을 수 있다.

![connections&connscan](https://github.com/RaGoon1222/antirootDigitalForensic/blob/master/week_1/img/connections&connscan.PNG?raw=true)

그런데 두 옵션의 결과가 다른 것을 볼 수 있다.  
이는 connections 옵션이 윈도우 보안패치 때문에 제대로 실행이 되지 않았기 때문이다.  
해당 도구는 여러가지 [플러그인](https://ko.wikipedia.org/wiki/%ED%94%8C%EB%9F%AC%EA%B7%B8%EC%9D%B8)을 추가할 수 있도록 되어있고, 이 도구를 모듈로 또 다른 도구를 제작할 수도 있다.  
